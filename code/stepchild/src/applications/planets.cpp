...
void Planet::advanceOneStep(){
  ...
  if(currentStep == stepsPerPeriod){
    currentStep = 0;
    stepchild.midi.noteOn(pitch,127,0);
    stepchild.midi.noteOff(pitch,127,0);
    sentNote = true;
  }
  else{
    sentNote = false;
...
void drawPlanetHighlight(Planet &p){
  stepchild.display.drawCircle(p.loc.x,p.loc.z,p.size+4+(millis()/200)%2,1);
}
...
bool testSolarSystem(){
  ...
  system.xOff = stepchild.SCREEN_WIDTH/2;
  system.yOff = stepchild.SCREEN_HEIGHT/2;
  system.angle = 90;
  ...
  while(true){
    ...
    if(stepchild.buttons.joystickX != 0){
      if(selectingPlanet && stepchild.itsbeen(200)){
        highlightedPlanet++;
    ...
    system.draw();
    stepchild.display.drawCircle(system.planets[highlightedPlanet].loc.x+system.xOff,system.planets[highlightedPlanet].loc.y+system.yOff,system.planets[highlightedPlanet].size+4+(millis()/200)%2,1);
