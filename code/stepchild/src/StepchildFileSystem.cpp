#ifndef HEADLESS

#include <Arduino.h>
#include <vector>
#include <algorithm>
#include "Stepchild.h"
#include "StepchildFileSystem.h"
#include "guiUtilities.h"
#include "stringPatch.h"

extern Stepchild stepchild;

using namespace std;

StepchildFileSystem::StepchildFileSystem(Stepchild* ptr){
  this->stepchild = ptr;
}
void StepchildFileSystem::writeFileHeader(File& f){
  f.write(sizeof(sequenceFileHeader));//number of sections in the file
  //converting FileFormatCode into uint8_t, this is dumb
  uint8_t temp[sizeof(sequenceFileHeader)];
  for(uint8_t i = 0; i<sizeof(sequenceFileHeader); i++){
    temp[i] = sequenceFileHeader[i];
  }
  f.write(temp,sizeof(temp));
}
void StepchildFileSystem::writeCurrentFileFormatToSerial(){
  Serial.write(sizeof(sequenceFileHeader));
  Serial.write((const uint8_t*)sequenceFileHeader,sizeof(sequenceFileHeader));
}
//lmfao
void StepchildFileSystem::writeFileFormatFromFileToSerial(File& f){
  uint8_t formatSize[1];
  f.read(formatSize,1);
  FileFormatCode fileHeader[formatSize[0]];
  f.read((uint8_t*)fileHeader,formatSize[0]);
  Serial.write(formatSize,1);
  Serial.write((uint8_t*)fileHeader,formatSize[0]);
}

//sends the number of bytes in the file
void StepchildFileSystem::sendByteCount(String filename){
  uint32_t byteCount = getByteCount(filename);
  uint8_t bytes[4] = {uint8_t(byteCount>>24),uint8_t(byteCount>>16),uint8_t(byteCount>>8),uint8_t(byteCount)};
  Serial.write(bytes,4);
}

void StepchildFileSystem::sendFileName(String filename){
  Serial.print('\n'+filename+'\n');
}

//returns Bytes of a file (must be called within LittleFS)
uint32_t StepchildFileSystem::getByteCount(String filename){
  uint32_t byteCount = 0;
  String path = "/SAVES/"+filename;
  File seqFile = LittleFS.open(path,"r");
  if(seqFile){
    uint32_t s = seqFile.size();
    seqFile.close();
    return s;
  }
  else
    return 0;
}

//returns a byteCount as a more recognizeable value, like 7kB or 5mB
String StepchildFileSystem::bytesToString(uint32_t bytes){
  //if it's less than 500, print bytes as-is
  if(bytes<500){
    return stringify(bytes)+"B";
  }
  //probably never going to go over this
  else if(bytes<500000){
    //divide by 100 so the decimal value is now in the 1's place
    bytes/= 100;
    String byteCount = stringify(bytes);
    byteCount += byteCount[byteCount.length()-1];
    byteCount[byteCount.length()-2] = '.';
    return byteCount+"kB";
  }
  else{
    return "bruh";
  }
}


uint32_t StepchildFileSystem::getByteCount_standAlone(String filename){
  LittleFS.begin();
  uint32_t byteCount = getByteCount(filename);
  LittleFS.end();
  return byteCount;
}
//loads files into menu, for browsing
//also returns each file's filesize
vector<String> StepchildFileSystem::loadFilesAndSizes(){
  vector<String> fileSizes;
  LittleFS.begin();
  if(LittleFS.exists("/SAVES")){
    Dir saves = LittleFS.openDir("/SAVES");
    while(saves.next()){
      fileSizes.push_back(saves.fileName()+" "+bytesToString(getByteCount(saves.fileName())));
    }
  }
  else{
  }
  LittleFS.end();
  return fileSizes;
}

vector<String> StepchildFileSystem::loadFiles(){
  vector<String> filenames = {"*new*"};
  LittleFS.begin();
  if(LittleFS.exists("/SAVES")){
    Dir saves = LittleFS.openDir("/SAVES");
    while(saves.next()){
      filenames.push_back(saves.fileName());
    }
  }
  else{
  }
  LittleFS.end();
  return filenames;
}

void StepchildFileSystem::renameSeqFile(String filename){
  String newName = enterText("new name?");
  filename = "/SAVES/"+filename;
  newName = "/SAVES/"+newName;
  LittleFS.begin();
  LittleFS.rename(filename,newName);
  LittleFS.end();
}

bool StepchildFileSystem::deleteSeqFile(String filename){
  int choice = binarySelectionBox(64,32,"NO","YEA","delete file?");
  if(choice == 1){
    filename = "/SAVES/"+filename;
    LittleFS.begin();
    LittleFS.remove(filename);
    LittleFS.end();
    return true;
  }
  return false;
}

void StepchildFileSystem::quickSave(){
  //if it hasn't been saved yet
  if(currentFilename == ""){
    String fileName = enterText("filename?");
    if(fileName != ""){
      writeSeqFile(fileName);
      currentFilename = fileName;
    }
  }
  //if there is a filename for it
  else{
    writeSeqFile(currentFilename);
  }
}

void StepchildFileSystem::duplicateSeqFile(String filename){
  LittleFS.begin();
  String path = "/SAVES/"+filename;
  File targetFile = LittleFS.open(path,"r");
  //copy file data into an array
  uint16_t size = targetFile.size();
  uint8_t data[size];
  targetFile.read(data,size);
  targetFile.close();
  String newPath = "/SAVES/"+filename+"_copy";
  File newFile = LittleFS.open(newPath,"w");
  //write array into new file
  newFile.write(data,size);
  newFile.close();
  LittleFS.end();
}
//loads settings from settings.txt
void StepchildFileSystem::loadSettings(){
  LittleFS.begin();
  if(LittleFS.exists("/settings")){
    loadSavedSettingsFromFile();
  }
  //if there isn't a file yet, create one with the current parameters
  else{
    writeCurrentSettingsToFile();
  }
  LittleFS.end();
}

//Reads through the fileFormat array and writes data according to the format order!
void StepchildFileSystem::writeSeqFile(String filename){
  LittleFS.begin();
  String path = "/SAVES/"+filename;
  File seqFile = LittleFS.open(path,"w");
  if(!seqFile)
    return;
  
  // return;
  writeFileHeader(seqFile);
  for(FileFormatCode code:sequenceFileHeader){
    switch(code){
      case SEQ_DATA:{
        uint8_t start[2] = {0,0};
        uint8_t end[2] = {uint8_t(this->stepchild->sequenceLength>>8),uint8_t(this->stepchild->sequenceLength)};
        seqFile.write(start,2);
        seqFile.write(end,2);
        break;}
      case TRACK_AND_NOTE_DATA:{
        uint8_t t[1] = {uint8_t(this->stepchild->trackData.size())};
        seqFile.write(t,1);
        for(int track = 0; track<this->stepchild->trackData.size(); track++){
          //(noteCount1)(noteCount2),(pitch),(channel),(muteGroup),(flags)
          uint8_t trackDat[6] = {uint8_t((this->stepchild->noteData[track].size()-1)>>8),uint8_t(this->stepchild->noteData[track].size()-1),uint8_t(this->stepchild->trackData[track].pitch),uint8_t(this->stepchild->trackData[track].channel),uint8_t(this->stepchild->trackData[track].muteGroup),this->stepchild->trackData[track].flags};
          seqFile.write(trackDat,6);
        }
        for(int track = 0; track<this->stepchild->trackData.size(); track++){
          for(int note = 1; note < this->stepchild->noteData[track].size(); note++){
            //notes are stored start, end,
            //and then vel, chance, flags
            //                              first 8 bits                          last 8 bits
            uint8_t notePosData[4] = {uint8_t(this->stepchild->noteData[track][note].startPos>>8),uint8_t(this->stepchild->noteData[track][note].startPos),uint8_t(this->stepchild->noteData[track][note].endPos>>8),uint8_t(this->stepchild->noteData[track][note].endPos)};
            uint8_t noteData[5] = {uint8_t(this->stepchild->noteData[track][note].velocity), uint8_t(this->stepchild->noteData[track][note].chance),this->stepchild->noteData[track][note].flags,this->stepchild->noteData[track][note].superposition.pitch,this->stepchild->noteData[track][note].superposition.odds};
            seqFile.write(notePosData,4);
            seqFile.write(noteData,5);
          }
        }
        break;}
      case AUTOTRACK_DATA:{
        //Write autotrack count
        uint8_t numOfAutotracks[1] = {uint8_t(this->stepchild->autotrackData.size())};
        seqFile.write(numOfAutotracks,1);
        //writing dt data
        for(uint8_t dt = 0; dt<numOfAutotracks[0]; dt++){
          uint8_t autotrackInfoData[4] = {this->stepchild->autotrackData[dt].control,this->stepchild->autotrackData[dt].channel,this->stepchild->autotrackData[dt].isActive,this->stepchild->autotrackData[dt].parameterType};
          uint8_t numberOfDataPoints[2] = {uint8_t(this->stepchild->autotrackData[dt].data.size()>>8),uint8_t(this->stepchild->autotrackData[dt].data.size())};
          uint8_t autotrackRawData[this->stepchild->autotrackData[dt].data.size()];
          for(uint16_t dataPoint = 0; dataPoint<this->stepchild->autotrackData[dt].data.size(); dataPoint++){
            autotrackRawData[dataPoint] = this->stepchild->autotrackData[dt].data[dataPoint];
          }
          seqFile.write(autotrackInfoData,4);
          seqFile.write(numberOfDataPoints,2);
          seqFile.write(autotrackRawData,this->stepchild->autotrackData[dt].data.size());
        }
        break;}
      case LOOP_DATA:{
        //writing loop data
        uint8_t numberOfLoops[1] = {uint8_t(this->stepchild->loopData.size())}; 
        seqFile.write(numberOfLoops,1);
        //loops are stored as
        //(number of loops),(start1),(start2),(end1),(end2),(iterations),(style)
        for(uint8_t loop = 0; loop<this->stepchild->loopData.size(); loop++){
          uint8_t start[2] = {uint8_t(this->stepchild->loopData[loop].start>>8),uint8_t(this->stepchild->loopData[loop].start)};
          uint8_t end[2] = {uint8_t(this->stepchild->loopData[loop].end>>8),uint8_t(this->stepchild->loopData[loop].end)};
          uint8_t loopDat[2] = {uint8_t(this->stepchild->loopData[loop].reps),uint8_t(this->stepchild->loopData[loop].type)};
          seqFile.write(start,2);
          seqFile.write(end,2);
          seqFile.write(loopDat,2);
        }
        break;}
      case CLOCK_DATA:{
        uint8_t bpmBytes[2] = {uint8_t(this->stepchild->clock.BPM>>8),uint8_t(this->stepchild->clock.BPM)};
        seqFile.write(bpmBytes,2);
        //swing amount
        uint8_t swingAmountBytes[2] = {uint8_t(uint16_t(this->stepchild->clock.swingCurve.amplitude)>>8),uint8_t(uint16_t(this->stepchild->clock.swingCurve.amplitude))};
        seqFile.write(swingAmountBytes,2);
        //swing subDiv
        uint8_t swingSubDivBytes[2] = {uint8_t(this->stepchild->clock.swingCurve.period>>8),uint8_t(this->stepchild->clock.swingCurve.period)};
        seqFile.write(swingSubDivBytes,2);
        //swing on/off
        uint8_t swingByte[1] = {uint8_t(this->stepchild->clock.isSwinging)};
        seqFile.write(swingByte,1);
        break;}
      case MIDI_PORT_DATA:{
        //writing routing data
        uint8_t midiChannelBytes[10] = {uint8_t(stepchild->midi.midiChannelFilters[0]>>8),uint8_t(stepchild->midi.midiChannelFilters[0]),
                                uint8_t(stepchild->midi.midiChannelFilters[1]>>8),uint8_t(stepchild->midi.midiChannelFilters[1]),
                                uint8_t(stepchild->midi.midiChannelFilters[2]>>8),uint8_t(stepchild->midi.midiChannelFilters[2]),
                                uint8_t(stepchild->midi.midiChannelFilters[3]>>8),uint8_t(stepchild->midi.midiChannelFilters[3]),
                                uint8_t(stepchild->midi.midiChannelFilters[4]>>8),uint8_t(stepchild->midi.midiChannelFilters[4])};
        seqFile.write(midiChannelBytes,10);
        //writing thru data
        uint8_t midiThruBytes[5];
        for(uint8_t port = 0; port<5; port++){
          midiThruBytes[port] = stepchild->midi.isThru(port);
        }
        seqFile.write(midiThruBytes,5);
        break;}
      case PC_DATA:{
        uint8_t PCSeqData[2];
        for(uint8_t port = 0; port<5; port++){
          PCSeqData[0] = uint8_t(this->stepchild->PCData[port].size()>>8);
          PCSeqData[1] = uint8_t(this->stepchild->PCData[port].size());
          seqFile.write(PCSeqData,2);
          for(uint16_t message = 0; message<this->stepchild->PCData[port].size(); message++){
            //channel, bank, subbank, val, timestep(2)
            uint8_t messageData[6] = {this->stepchild->PCData[port][message].channel,this->stepchild->PCData[port][message].bank,this->stepchild->PCData[port][message].subBank,this->stepchild->PCData[port][message].val,uint8_t(this->stepchild->PCData[port][message].timestep>>8),uint8_t(this->stepchild->PCData[port][message].timestep)};
            seqFile.write(messageData,6);
          }
        }
        break;}
    }
  }
  seqFile.close();
  LittleFS.end();
}

//helper function for writing bytes to the serial buffer
void writeBytesToSerial(uint8_t* byteArray, uint16_t numberOfBytes){
#ifndef HEADLESS
  Serial.write(byteArray,numberOfBytes);
#endif
}

void StepchildFileSystem::writeCurrentSeqToSerial(bool waitForResponse){
  //clearing serial buffer
  Serial.flush();
  writeCurrentFileFormatToSerial();
  for(FileFormatCode data:sequenceFileHeader){
    switch(data){
      case SEQ_DATA:{
        uint8_t start[2] = {0,0};
        uint8_t end[2] = {uint8_t(this->stepchild->sequenceLength>>8),uint8_t(this->stepchild->sequenceLength)};
        writeBytesToSerial(start,2);
        writeBytesToSerial(end,2);
        break;}
      case TRACK_AND_NOTE_DATA:{
        //writing track info
        //(number of tracks)
        uint8_t t[1] = {uint8_t(this->stepchild->trackData.size())};
        writeBytesToSerial(t,1);
        for(int track = 0; track<this->stepchild->trackData.size(); track++){
          //(noteCount1)(noteCount2),(pitch),(channel),(latched),(muteGroup),(primed)
          uint8_t trackDat[7] = {uint8_t((this->stepchild->noteData[track].size()-1)>>8),uint8_t(this->stepchild->noteData[track].size()-1),uint8_t(this->stepchild->trackData[track].pitch),uint8_t(this->stepchild->trackData[track].channel),uint8_t(this->stepchild->trackData[track].isLatched()),uint8_t(this->stepchild->trackData[track].muteGroup),uint8_t(this->stepchild->trackData[track].isPrimed())};
          writeBytesToSerial(trackDat,7);
        }
        //writing note info
        for(int track = 0; track<this->stepchild->trackData.size(); track++){
          for(int note = 1; note < this->stepchild->noteData[track].size(); note++){
            //notes are stored start, end,
            //and then vel, chance, selected, muted
            //                              first 8 bits                          last 8 bites
            uint8_t notePosData[4] = {uint8_t(this->stepchild->noteData[track][note].startPos>>8),uint8_t(this->stepchild->noteData[track][note].startPos),uint8_t(this->stepchild->noteData[track][note].endPos>>8),uint8_t(this->stepchild->noteData[track][note].endPos)};
            uint8_t noteData[4] = {uint8_t(this->stepchild->noteData[track][note].velocity), uint8_t(this->stepchild->noteData[track][note].chance), uint8_t(this->stepchild->noteData[track][note].isSelected()), uint8_t(this->stepchild->noteData[track][note].isMuted())};
            writeBytesToSerial(notePosData,4);
            writeBytesToSerial(noteData,4);
          }
        }
        break;}
      case AUTOTRACK_DATA:{
        //writing autotrack info, if there are any autotracks
        //number of bytes is split in two! because it's a uint16_t
        //stored as (number of autotracks),(control),(channel),(isActive),(number of bytes1),(number of bytes2),(data)...
        //number of autotracks is always written, even if it's zero!
        uint8_t numOfAutotracks[1] = {uint8_t(this->stepchild->autotrackData.size())};
        writeBytesToSerial(numOfAutotracks,1);
        if(this->stepchild->autotrackData.size()>0){      
          //writing dt data
          for(uint8_t dt = 0; dt<numOfAutotracks[0]; dt++){
            uint8_t autotrackInfoData[3] = {this->stepchild->autotrackData[dt].control,this->stepchild->autotrackData[dt].channel,this->stepchild->autotrackData[dt].isActive};
            uint8_t numberOfDataPoints[2] = {uint8_t(this->stepchild->autotrackData[dt].data.size()>>8),uint8_t(this->stepchild->autotrackData[dt].data.size())};
            uint8_t autotrackRawData[this->stepchild->autotrackData[dt].data.size()];
            for(uint16_t dataPoint = 0; dataPoint<this->stepchild->autotrackData[dt].data.size(); dataPoint++){
              autotrackRawData[dataPoint] = this->stepchild->autotrackData[dt].data[dataPoint];
            }
            writeBytesToSerial(autotrackInfoData,3);
            writeBytesToSerial(numberOfDataPoints,2);
            writeBytesToSerial(autotrackRawData,this->stepchild->autotrackData[dt].data.size());
          }
        }
        break;}
      case LOOP_DATA:{
        //writing loop data
        uint8_t numberOfLoops[1] = {uint8_t(this->stepchild->loopData.size())}; 
        writeBytesToSerial(numberOfLoops,1);
        if(this->stepchild->loopData.size()>0){
          //loops are stored as
          //(number of loops),(start1),(start2),(end1),(end2),(iterations),(style)
          for(uint8_t loop = 0; loop<this->stepchild->loopData.size(); loop++){
            uint8_t start[2] = {uint8_t(this->stepchild->loopData[loop].start>>8),uint8_t(this->stepchild->loopData[loop].start)};
            uint8_t end[2] = {uint8_t(this->stepchild->loopData[loop].end>>8),uint8_t(this->stepchild->loopData[loop].end)};
            uint8_t loopDat[2] = {uint8_t(this->stepchild->loopData[loop].reps),uint8_t(this->stepchild->loopData[loop].type)};
            writeBytesToSerial(start,2);
            writeBytesToSerial(end,2);
            writeBytesToSerial(loopDat,2);
          }
        }
        break;}
      case CLOCK_DATA:{
        //writing clock data
        //this->stepchild->clock.BPM
        uint8_t bpmBytes[2] = {uint8_t(this->stepchild->clock.BPM>>8),uint8_t(this->stepchild->clock.BPM)};
        writeBytesToSerial(bpmBytes,2);
        //swing amount
        uint8_t swingAmountBytes[2] = {uint8_t(this->stepchild->clock.swingCurve.amplitude>>8),uint8_t(this->stepchild->clock.swingCurve.amplitude)};
        writeBytesToSerial(swingAmountBytes,2);
        //swing subDiv
        uint8_t swingSubDivBytes[2] = {uint8_t(this->stepchild->clock.swingCurve.period>>8),uint8_t(this->stepchild->clock.swingCurve.period)};
        writeBytesToSerial(swingSubDivBytes,2);
        //swing on/off
        uint8_t swingByte[1] = {uint8_t(this->stepchild->clock.isSwinging)};
        writeBytesToSerial(swingByte,1);
        break;}
      case MIDI_PORT_DATA:{
        //writing routing data
        uint8_t midiChannelBytes[10] = {uint8_t(stepchild->midi.midiChannelFilters[0]>>8),uint8_t(stepchild->midi.midiChannelFilters[0]),
                                uint8_t(stepchild->midi.midiChannelFilters[1]>>8),uint8_t(stepchild->midi.midiChannelFilters[1]),
                                uint8_t(stepchild->midi.midiChannelFilters[2]>>8),uint8_t(stepchild->midi.midiChannelFilters[2]),
                                uint8_t(stepchild->midi.midiChannelFilters[3]>>8),uint8_t(stepchild->midi.midiChannelFilters[3]),
                                uint8_t(stepchild->midi.midiChannelFilters[4]>>8),uint8_t(stepchild->midi.midiChannelFilters[4])};
        writeBytesToSerial(midiChannelBytes,10);
        //writing thru data
        uint8_t midiThruBytes[5];
        for(uint8_t port = 0; port<5; port++){
          midiThruBytes[port] = stepchild->midi.isThru(port);
        }
        writeBytesToSerial(midiThruBytes,5);
        break;}
      case PC_DATA:{
        uint8_t PCSeqData[2];
        for(uint8_t port = 0; port<5; port++){
          PCSeqData[0] = uint8_t(this->stepchild->PCData[port].size()>>8);
          PCSeqData[1] = uint8_t(this->stepchild->PCData[port].size());
          writeBytesToSerial(PCSeqData,2);
          for(uint16_t message = 0; message<this->stepchild->PCData[port].size(); message++){
            //channel, bank, subbank, val, timestep(2)
            uint8_t messageData[6] = {this->stepchild->PCData[port][message].channel,this->stepchild->PCData[port][message].bank,this->stepchild->PCData[port][message].subBank,this->stepchild->PCData[port][message].val,uint8_t(this->stepchild->PCData[port][message].timestep>>8),uint8_t(this->stepchild->PCData[port][message].timestep)};
            writeBytesToSerial(messageData,6);
          }
        }
        break;}
    }
  }
}

void StepchildFileSystem::loadSeqFile(String filename){
  LittleFS.begin();
  String path = "/SAVES/"+filename;
  File seqFile = LittleFS.open(path,"r");
  if(seqFile){
    uint8_t headerSectionCount[1];
    seqFile.read(headerSectionCount,1);
    FileFormatCode fileFormat[headerSectionCount[0]];
    seqFile.read((uint8_t*)fileFormat,sizeof(fileFormat));
    //replacing sequence with empty data
    this->stepchild->eraseSequence();
    for(FileFormatCode data:fileFormat){
      switch(data){
        case SEQ_DATA:{
          //start, end
          uint8_t start[2];
          uint8_t end[2];
          seqFile.read(start,2);
          seqFile.read(end,2);
          this->stepchild->sequenceLength = (uint16_t(end[0])<<8)+uint16_t(end[1]);
          break;}
        case TRACK_AND_NOTE_DATA:{
          //loading tracks
          uint8_t tracks[1];
          seqFile.read(tracks,1);
          vector<uint16_t> noteCount;
          for(int i = 0; i<tracks[0]; i++){
            uint8_t trackDat[6];//notecount1, notecount2, pitch, channel, muteGroup, flags
            seqFile.read(trackDat,6);
            Track newTrack;

            newTrack.pitch = trackDat[2];
            newTrack.channel = trackDat[3];
            newTrack.muteGroup = trackDat[4];
            newTrack.flags = trackDat[5];

            this->stepchild->addTrack(newTrack,false);

            noteCount.push_back((uint16_t(trackDat[0])<<8)+trackDat[1]);
          }
          //you need to reverse the tracks... for some reason
          reverse(this->stepchild->trackData.begin(),this->stepchild->trackData.end());

          //loading notes
          for(uint8_t track = 0; track<this->stepchild->trackData.size(); track++){
            for(uint16_t note = 0; note<noteCount[track]; note++){
              //notes are stored start, end,
              //and then vel, chance, flags, superpos1,superpos2
              uint8_t notePosData[4];
              uint8_t noteData[5];
              seqFile.read(notePosData,4);
              seqFile.read(noteData,5);
              uint16_t notePos[2] = {uint16_t((notePosData[0]<<8)+notePosData[1]),uint16_t((notePosData[2]<<8)+notePosData[3])};
              Note newNote = Note(notePos[0],notePos[1],noteData[0],noteData[1],noteData[2]);
              newNote.superposition = Superposition(noteData[3],noteData[4]);
              this->stepchild->loadNote(newNote,track);
            }
          }
          break;}
        case AUTOTRACK_DATA:{
          //loading DT's
          uint8_t numberOfDts[1];
          seqFile.read(numberOfDts,1);
          //clear out this->stepchild->autotrackData
          vector<Autotrack> newData;
          newData.swap(this->stepchild->autotrackData);
          if(numberOfDts[0]>0){
            for(uint8_t dt = 0; dt<numberOfDts[0]; dt++){
              //control, channel, isActive
              uint8_t dtInfo[4];
              uint8_t numberOfPoints[2];
              seqFile.read(dtInfo,4);
              seqFile.read(numberOfPoints,2);
              uint16_t numberOfDtPoints = uint16_t((numberOfPoints[0]<<8)+numberOfPoints[1]);

              //getting points
              uint8_t points[numberOfDtPoints];
              seqFile.read(points,numberOfDtPoints);
              vector<uint8_t> temp;
              for(uint16_t point = 0; point<numberOfDtPoints; point++){
                temp.push_back(points[point]);
              }
              this->stepchild->createAutotrack(dtInfo[0],dtInfo[1],dtInfo[2],temp,dtInfo[3]);
            }
          }
          break;}
        case LOOP_DATA:{
          //loading loops
          uint8_t numberOfLoops[1];
          seqFile.read(numberOfLoops,1);
          vector<Loop> newLoopData;
          for(uint8_t loop = 0; loop<numberOfLoops[0]; loop++){
            uint8_t start[2];
            uint8_t end[2];
            uint8_t loopInfo[2];
            seqFile.read(start,2);
            seqFile.read(end,2);
            seqFile.read(loopInfo,2);
            Loop loopDat;
            loopDat.start = uint16_t((start[0]<<8)+start[1]);
            loopDat.end = uint16_t((end[0]<<8)+end[1]);
            loopDat.reps = loopInfo[0];
            loopDat.type = static_cast<LoopType>(loopInfo[1]);
            newLoopData.push_back(loopDat);
          }
          this->stepchild->loopData.swap(newLoopData);
          break;}
        case CLOCK_DATA:{
          //loading clock data
          uint8_t bpmBytes[2];
          uint8_t swingAmountBytes[2];
          uint8_t swingSubDivBytes[2];
          uint8_t swingByte[1];
          seqFile.read(bpmBytes,2);
          seqFile.read(swingAmountBytes,2);
          seqFile.read(swingSubDivBytes,2);
          seqFile.read(swingByte,1);
          this->stepchild->clock.setBPM(uint16_t(bpmBytes[0]<<8) + uint16_t(bpmBytes[1]));
          this->stepchild->clock.swingCurve.amplitude = float(uint16_t(swingAmountBytes[0]<<8) + uint16_t(swingAmountBytes[1]));
          this->stepchild->clock.swingCurve.period = uint16_t(swingSubDivBytes[0]<<8) + uint16_t(swingSubDivBytes[1]);
          this->stepchild->clock.isSwinging = swingByte[0];
          break;}
        case MIDI_PORT_DATA:{
          //loading routing/thru data
          uint8_t midiChannelBytes[10];
          uint8_t midiThruBytes[5];
          seqFile.read(midiChannelBytes,10);
          seqFile.read(midiThruBytes,5);
          for(uint8_t port = 0; port<5; port++){
            stepchild->midi.midiChannelFilters[port] = uint16_t(midiChannelBytes[port*2]<<8)+uint16_t(midiChannelBytes[port*2+1]);
            stepchild->midi.setThru(port,bool(midiThruBytes[port]));
          }
          break;}
        case PC_DATA:{
          //loading PC sequence data
          uint8_t PCSeqData[2];
          for(uint8_t port = 0; port<5; port++){
            seqFile.read(PCSeqData,2);
            uint16_t messageCount = (PCSeqData[0]<<8)+PCSeqData[1];
            for(uint16_t message = 0; message<messageCount; message++){
              //channel, bank, subbank, val, timestep(2)
              uint8_t messageData[6];
              seqFile.read(messageData,6);
              this->stepchild->PCData[port].push_back(ProgramChange(messageData[0],messageData[1],messageData[2],messageData[3],(messageData[4]<<8)+messageData[5]));
            }
          }
          break;}
      }
    }
    seqFile.close();
    LittleFS.end();
    this->stepchild->setActiveTrack(0,false);
  }
  else{
    alert("Error opening file!",1000);
  }
}



//Settings ------
/*
display pitches/numbers for tracks
leds on/off
external/internal clock
continue playing after recording y/n
wait for note, or count in, for recording
time before screensaver/sleep --> this needs to be a 16-bit number
*/

void StepchildFileSystem::writeCurrentSettingsToFile(){
  LittleFS.begin();
  File f = LittleFS.open("/settings","w");
  for(uint8_t which:settingsFileHeader){
    switch(which){
      case TRACK_DISPLAY:{
        uint8_t pOrN[1] = {this->stepchild->pitchesOrNumbers};
        f.write(pOrN,1);
        break;}
      case LEDS_ACTIVE:{
        uint8_t leds[1] = {this->stepchild->buttons.LEDsActive};
        f.write(leds,1);
        break;}
      case CLOCK_SOURCE:{
        uint8_t clockData[1] = {this->stepchild->clock.clockSource};
        f.write(clockData,1);
        break;}
      case REC_MODE:{
        uint8_t playAfterRec[1] = {0};
        f.write(playAfterRec,1);
        break;}
      case REC_TRIGGER:{
        uint8_t countIn[1] = {stepchild->waitForNoteBeforeRec};
        f.write(countIn,1);
        break;}
      case SLEEP_TIME:{
        uint8_t idleTime[2] = {uint8_t(stepchild->sleepTime>>8),uint8_t(stepchild->sleepTime)};
        f.write(idleTime,2);
        break;}
      case SCREEN_BRIGHTNESS:{
        uint8_t brightness[1] = {stepchild->screenBrightness};
        f.write(brightness,1);
        break;}
    }
  }
  f.close();
  LittleFS.end();
}

void StepchildFileSystem::loadSavedSettingsFromFile(){
  LittleFS.begin();
  File f = LittleFS.open("/settings","r");
  if(!f)
    return;
  for(uint8_t which:settingsFileHeader){
    switch(which){
      case TRACK_DISPLAY:{
        uint8_t pOrN[1];
        f.read(pOrN,1);
        this->stepchild->pitchesOrNumbers = pOrN[0];
        break;}
      case LEDS_ACTIVE:{
        uint8_t leds[1];
        f.read(leds,1);
        this->stepchild->buttons.LEDsActive = leds[0];
        break;}
      case CLOCK_SOURCE:{
        uint8_t clockData[1];
        f.read(clockData,1);
        if(clockData[0] == 0)
          this->stepchild->clock.clockSource = INTERNAL_CLOCK;
        else
          this->stepchild->clock.clockSource = EXTERNAL_CLOCK;
        break;}
      case REC_MODE:{
        uint8_t playAfterRec[1];
        f.read(playAfterRec,1);
        break;}
      case REC_TRIGGER:{
        uint8_t countIn[1];
        f.read(countIn,1);
        stepchild->waitForNoteBeforeRec = countIn[0];
        break;}
      case SLEEP_TIME:{
        uint8_t idleTime[2];
        f.read(idleTime,2);
        stepchild->sleepTime = (uint16_t(idleTime[0])<<8) | idleTime[1];
        break;}
      case SCREEN_BRIGHTNESS:{
        uint8_t brightness[1];
        f.read(brightness,1);
        if(stepchild->screenBrightness != brightness[0]){
          stepchild->display.ssd1306_command(SSD1306_SETCONTRAST);
          stepchild->display.ssd1306_command(stepchild->screenBrightness);
          stepchild->screenBrightness = brightness[0];
        }
        break;}
    }
  }
  LittleFS.end();
}

//just writes all the file data to the serial buffer
void StepchildFileSystem::writeFileToSerial(File f){
    uint32_t fileSize = f.size();
    uint8_t byteCount[4] = {uint8_t(fileSize>>24),uint8_t(fileSize>>16),uint8_t(fileSize>>8),uint8_t(fileSize)};
    Serial.write(byteCount,4);//write the filesize

    char data[fileSize];
    f.readBytes(data,fileSize);//read the file into a buffer
    Serial.write(data,fileSize);//write the buffer to the serial buffer
    Serial.flush();//wait for data to be sent
}

//For headless mode
#else
  void flashTest(){
    return;
  }
  void writeSeqFile(String fname){
    return;
  }
//  void writeBytesToSerial(uint8_t* byteArray, uint16_t number){
//    return;
//  }
  void writeCurrentSeqToSerial(bool w){
    return;
  }
  void sendByteCount(String filename){
    return;
  }
  void sendFileName(String filename){
    return;
  }
  uint32_t getByteCount_standAlone(String filename){
    return 0;
  }
  uint32_t getByteCount(String filename){
    return 0;
  }
  void loadSeqFile(String filename){
    return;
  }
  vector<String> loadFilesAndSizes(){
    vector<String> fileSizes = {"0"};
    return fileSizes;
  }
  vector<String> loadFiles(){
    vector<String> filenames = {"*new*","test","beebebebbe","idk","i love you","sp404","microKORG"};
    return filenames;
  }
  void renameSeqFile(String filename){
    return;
  }
  bool deleteSeqFile(String filename){
    return false;
  }
  void duplicateSeqFile(String filename){
    return;
  }
  void writeCurrentSettingsToFile(){
  }

  void loadSavedSettingsFromFile(){
  }
  void loadSettings(){
  }
#endif
