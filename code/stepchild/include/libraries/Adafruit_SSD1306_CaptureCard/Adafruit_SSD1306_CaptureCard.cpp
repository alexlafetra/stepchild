...
#include "Adafruit_SSD1306_CaptureCard.h"
#include "splash.h"
...
//this version JUST prints the buffer to the USB serial port
void Adafruit_SSD1306::displayToSerial(void) {
  if(updated && (millis()-timeLastFrame)>10){
    timeLastFrame = millis();
    Serial.write(this->getBuffer(),1024);
    // Serial.write(170); //send a 10101010 (unlikely to show up graphically, but not impossible!)
    //Turns out, waiting 10ms is enough to clear the serial buffer!
    updated = false;
  }
}
#endif
void Adafruit_SSD1306::display(void) {
  //If you're in capture card mode, write the screen to serial first
  #ifdef CAPTURECARD
    displayToSerial();
  #endif
  TRANSACTION_START
