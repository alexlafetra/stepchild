...
std::vector<uint8_t> extractLetter(const uint8_t* spriteMap, int spriteMapWidth, int letterWidth, int letterHeight, int letterIndexX, int letterIndexY) {
    std::vector<uint8_t> letterPixels(letterWidth * letterHeight);

    for (int y = 0; y < letterHeight; ++y) {
        for (int x = 0; x < letterWidth; ++x) {
            int spriteX = letterIndexX * letterWidth + x;
            int spriteY = letterIndexY * letterHeight + y;
            int spriteIndex = spriteY * spriteMapWidth + spriteX;

            letterPixels[y * letterWidth + x] = spriteMap[spriteIndex];
        }
    }
...
void printCursiveLetter(uint8_t letter){

  const uint8_t bitmap[6][6] 
